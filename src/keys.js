...
const PLANS = {
  free: { name: 'Free', validationsPerDay: 20, batchLimit: 5, ratePerMinute: 5 },
  starter: { name: 'Starter', validationsPerDay: 500, batchLimit: 25, ratePerMinute: 30 },
  pro: { name: 'Pro', validationsPerDay: 5000, batchLimit: 50, ratePerMinute: 100 },
  business: { name: 'Business', validationsPerDay: 50000, batchLimit: 100, ratePerMinute: 500 },
};
...
const createKey = (plan = 'pro', email = null, stripeCustomerId = null) => {
  const keys = loadKeys();
  const apiKey = `ncci_${randomUUID().replace(/-/g, '')}`;
